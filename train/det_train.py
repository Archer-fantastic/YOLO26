...
def parse_args():
    ...
    # 训练核心参数
    parser.add_argument("--epochs", type=int, default=100, help="训练轮次（快速验证设为2，正常训练30-300）")
    parser.add_argument("--imgsz", type=int, default=640, help="输入图片尺寸（需为32的倍数，检测常用640）")
    parser.add_argument("--batch", type=int, default=16, help="批次大小（显存不足减小，验证时设小一点）")
    ...
    # 缓存参数（核心新增）
    parser.add_argument(
        "--cache", type=str, default="ram", help="数据集缓存方式：True/ram=内存缓存，disk=磁盘缓存，False=不缓存"
    )
    parser.add_argument(
        "--no-cache", dest="cache", action="store_const", const="false", help="关闭缓存（等价于 --cache false）"
    )
