...
def parse_args():
    ...
    # 数据集配置
    parser.add_argument("--data", type=str, required=True, help="数据集配置文件（dataset.yaml）路径")
    # 模型配置
    parser.add_argument(
        "--model", type=str, default=None, help="模型配置文件（如 yolo26n-seg.yaml 或 .pt），不传则使用 --weights"
    )
