...
def parse_args():
    ...
    parser.add_argument(
        "--folder",
        type=str,
        default=r"D:\Min\Projects\VSCodeProjects\dataset\instance_HD\测试集",
        help="单个文件夹路径（可选）",
    )
    ...
    parser.add_argument(
        "--save",
        type=str,
        default=r"D:\Min\Projects\VSCodeProjects\dataset\instance_HD\instance_HD_yolo\yolov11n\train\测试结果",
        help="结果保存根目录",
    )
...
def main():
    ...
    predictor = YOLOSegmentPredictor(
        args.model, draw_chinese=args.draw_chinese, save_vis=not args.no_vis, draw_box=args.box, draw_label=args.label
    )
