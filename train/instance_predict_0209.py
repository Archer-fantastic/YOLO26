...
class YOLOSegmentPredictor:
    def __init__(self, model_path, logger, draw_chinese=False, save_vis=False, draw_box=False, draw_label=False):
        ...
        self.logger.info(f"已加载实例分割模型：{model_path}，包含类别：{list(self.class_names.values())}")
        self.logger.info(f"中文标签绘制：{'开启' if draw_chinese else '关闭'}")
        self.logger.info(f"可视化结果保存：{'开启' if save_vis else '关闭'}")
        self.logger.info(f"绘制矩形框：{'开启' if draw_box else '关闭'}")
        self.logger.info(f"绘制文字标签：{'开启' if draw_label else '关闭'}")
    ...
    def _load_chinese_font(self):
            ...
            for f in fontManager.ttflist:
                for candidate in candidates:
                    if candidate.lower() in f.name.lower():
                        self.logger.info(f"通过 matplotlib 找到字体：{f.name} ({f.fname})")
                        return ImageFont.truetype(f.fname, 30)
        except Exception as e:
            self.logger.info(f"matplotlib 字体查找失败: {e}")
        ...
        for font_name in common_fonts:
            try:
                ...
                self.logger.info(f"成功加载字体：{font_name}")
                return font
            except Exception:
                continue
        ...
        self.logger.warning("未找到中文字体，中文标签将显示为 '????'")
    ...
    def predict_single_image(self, img_path, save_dir=None, vis_dir=None):
        ...
        if save_dir:
            ...
            if self.save_vis and vis_dir:
                ...
                if img is None or vis_img is None:
                    self.logger.warning(f"可视化图保存失败（读取图片失败）：{img_path}")
                else:
                    try:
                        concat_vis = np.hstack([img, vis_img])
                        cv2.imwrite(vis_path, concat_vis)
                    except Exception as e:
    ...
    def predict_single_folder(self, folder_path, save_root=None, recursive=False):
        ...
        if not img_paths:
            self.logger.warning(f"文件夹中未找到图像 - {folder_path}")
            return []

        self.logger.info(f"{'递归' if recursive else '非递归'}处理文件夹：{folder_path}，共{len(img_paths)}张图像")
        all_results = []
...
def main():
    ...
    log_file = os.path.join(save_root if save_root else os.getcwd(), "predict.log")
