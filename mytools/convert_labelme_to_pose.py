...
class Labelme2PoseYOLO:
    ...
    def convert_single(self, json_file, image_file):
        """转换单个标注文件，所有关键点合并为一行 (class_id=0) 返回标签字符串，无有效标注返回 None.
        """
        try:
            with open(json_file, encoding="utf-8") as f:
                data = json.load(f)
...
def main():
    ...
    parser.add_argument(
        "--dst", type=str, default=r"Z:\14-调试数据\lxm\Dataset\pose_涂布抓边\抓边模型_yolo_data", help="目标输出目录"
    )
    parser.add_argument("--mapping-file", type=str, default=None, help="label.ini 映射文件路径 (格式: 源标签 目标标签)")
    ...
    converter = Labelme2PoseYOLO(args.src, args.dst, keypoint_map=keypoint_map, val_split=args.val_split)
    converter.convert()
