...
def split_data(img_dir, label_dir, split_dir, split_ratio=[0.8, 0.2, 0.0], seed=42, link_mode="copy"):
    ...
    print(
        f"划分计划 -> 训练集: {train_point}, 验证集: {valid_point - train_point}, 测试集: {image_count - valid_point}"
    )

    # 模式显示中文映射
    mode_map = {"copy": "复制文件 (Copy)", "symlink": "软链接 (Symlink)", "hardlink": "硬链接 (Hardlink)"}
    print(f"传输模式: {mode_map.get(link_mode, link_mode)}")

    # 统计计数器
    stats = {"total": Counter(), "train": Counter(), "val": Counter(), "test": Counter()}
        ...
        if mode == "copy":
            ...
            try:
                ...
                if hasattr(e, "winerror") and e.winerror == 1314:
                    print(
                        "Error: Creating symlink failed (Permission denied). Try running as Administrator or use --link-mode copy."
                    )
                    raise e
                else:
                    raise e
    ...
    for i in tqdm(range(image_count), desc=f"正在处理 {os.path.basename(img_dir)}"):
        ...
        if src_label_path and os.path.exists(src_label_path):
            ...
            with open(dst_label_path, "w", encoding="utf-8"):
                pass
...
def main():
    ...
    if args.recursive:
        ...
        print("Mode: Single directory split")
        # 获取目录下所有图片
        img_exts = {".jpg", ".jpeg", ".png", ".bmp", ".tif", ".tiff", ".webp"}
        all_images = [
...
def parse_args():
    ...
    parser.add_argument(
        "--save-dir",
        type=str,
        default=r"Z:\14-调试数据\lxm\Dataset\极片类通用数据\yolo_dataset",
        help="划分后数据集保存路径",
    )
    parser.add_argument(
        "--label-dir", type=str, default=None, help="原label数据集路径 (非递归模式下，默认为 img_dir，即与图片同级)"
    )
