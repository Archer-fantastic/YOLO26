...
def get_short_name(filename, limit=250):
    """生成短文件名（保留扩展名，添加hash以防冲突） 策略：保留前缀(30) + Hash(8) + 后缀.
    """
...
def shape2points(shape, height, width):
    """将LabelMe的shape转换为归一化的点列表 参考自 lab2yolo.py.
    """
        ...
        if is_mapping:
            ...
            for line in lines:
                ...
                if len(parts) >= 2:
                    ...
                    target = parts[
                        -1
                    ]  # Assume last column is target (handling spaces in source? No, usually split by whitespace implies source is first token)
...
def main():
    ...
    if rename_count > 0:
        ...
        print("  注意：生成的 TXT 文件名已缩短，但原始图片文件名未修改。")
        print("  请务必根据生成的映射表重命名对应的图片，否则 YOLO 无法匹配！")
    ...
    if args.save_dir and duplicate_files:
        print("-" * 30)
        print("⚠️  警告: 发现重名文件冲突 (Output Directory Conflict):")
        print("  指定了统一保存目录，但不同子目录下存在同名文件，导致互相覆盖。")
...
def parse_args():
    ...
    parser.add_argument(
        "--json-dir",
        type=str,
        default=r"Z:\14-调试数据\lxm\Dataset\极片类通用数据",
        help="JSON文件夹路径 (或包含JSON的根目录)",
    )
    parser.add_argument("--save-dir", type=str, default=None, help="保存TXT的文件夹路径 (默认: 与JSON文件同目录)")

    parser.add_argument("--classes", type=str, nargs="+", help="类别名称列表 (按ID顺序)，例如: person car dog")
    parser.add_argument(
        "--class-file",
        type=str,
        default=r"Z:\14-调试数据\lxm\Dataset\极片类通用数据\labels.ini",
    ...
    parser.add_argument("--recursive", action="store_false", help="是否递归查找子目录")

    parser.add_argument(
        "--empty-label", action="store_false", help="为没有JSON标注的图片生成空的TXT标签文件 (作为背景图参与训练)"
    )
